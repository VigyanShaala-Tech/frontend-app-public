...
function addPlugins(config, slotName, plugins) {
  if (!(slotName in config.pluginSlots)) {
...
const modifyMainMenu = (widget) => {
  ...
  const LMS_BASE_URL = appConfig.LMS_BASE_URL;
  ...
  widget.content.menu = [
    ...
    },
    {
      ...
      submenuContent: (
        <>
          <a className="dropdown-item" href={`${PUBLIC_ROOT}/story`}>
            {intl.formatMessage(messages['public.header.nav.ourStory'])}
          </a>
          <a className="dropdown-item" href={`${PUBLIC_ROOT}/team`}>
            {intl.formatMessage(messages['public.header.nav.team'])}
          </a>
          <a className="dropdown-item" href={`${PUBLIC_ROOT}/supporter`}>
            {intl.formatMessage(messages['public.header.nav.supporters'])}
          </a>
          <a className="dropdown-item" href={`${PUBLIC_ROOT}/financial`}>
...
async function setConfig() {
  ...
  try {
    const { PLUGIN_OPERATIONS } = await import('@openedx/frontend-plugin-framework');
